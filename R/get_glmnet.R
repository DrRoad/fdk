...
#'  get_glmnet()
#' }
get_glmnet <- function(.data, yvar, parameter){
  ...
  time_weights_tmp <- get_time_weights(yvar = pull(select(.data, {{yvar}})), time_weight = parameter[["glmnet"]][["time_weight"]])
  ...
  if(is.null(parameter[["glmnet"]][["job"]][["reg_excluded"]])==FALSE){
    max_date <- max(.data[[parameter$glmnet$job$date_var]])
    .data <- .data[,!(names(.data) %in% parameter[["glmnet"]][["job"]][["var_excluded"]])]
  ...
  if(config[["glmnet"]][["optim_lambda"]]==FALSE){
    ...
    return(list(model_summary = list(train_size = length(.data[,1][[1]])
                ...
                , config = config
                , param = list(alpha = parameter[["glmnet"]][["alpha"]]
                               , lambda = parameter[["glmnet"]][["lambda"]]
    ...
    fit_tmp <- cv.glmnet(x = x_train, y = y_train
                         , alpha = parameter_grid[["glmnet"]][["alpha"]]
...
parameter <- list(glmnet = list(time_weight = 0.9, trend_discount = .9, alpha = 0, lambda = 0
                                , job = list(lag = 4
                                             , var_excluded = c("date", "forecast_item")
                                             , date_var = "date"
                                             , optim_lambda = FALSE)))


frequency_switcher <- function(frequency){
  
}



get_regressors <- function(.data, date_var, frequency){
  if(frequency == 12){
...
get_regressors(demo_1, date = date, frequency = 52)
...
# 
# demo_.data <- read_rds("../demo_.data.rds") %>%
#   mutate(month = months(date, abbr = T) %>% as.factor()
#          , trend = 1:n()) %>% 
#   select(y = volume, date, trend, month, reg=oos)
# 
# splits <- ts_split(.data = demo_.data, test_size = 6, lag = 4)
# 
...
# 
# fit_1 <- train %>% 
#   get_glmnet(.data = ., parameter = list(glmnet = parameter[[1]]), config = config) %>% 
...
#   map(.x = ., .f = get_glmnet(.data = .x[["train"]], parameter = parameter, job_config = config) %>% 
