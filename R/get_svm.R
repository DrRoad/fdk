...
get_svm <- function(.data, y_var, parameter = NULL){
  ...
  # New data
  new_data <- data.frame(trend = xd$trend, seasonal_var = xd$seasonal_var)
  ...
  if (class(Errorcheck)== "try-error") { 
    ...
    predicted <- predict(model_fit, new_data)
    svm_param <- paste0("Cost:",round(model_fit$cost,2),"; ","Gamma:",round(model_fit$gamma,2),"; ","Epsilon:",round(model_fit$epsilon,2))
  }
  # Timelapse
