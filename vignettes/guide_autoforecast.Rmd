...
AutoForecast is available as Gitlab's code repository, and it can be accessed for Sanofi employees using the company's network. There are ways to install it:
...
ap_init <- AirPassengers %>% 
  as_tsibble() %>%
  mutate(index = as.Date(index), causal_fact = "NA", causal_fact_value = 0, key = "ap") %>% 
  as_tibble()
